...
const NAV_GROUPS = [
  {
    label: "General",
    items: [
      { view: "dashboard", label: "成本看板", icon: "📊" },
      { view: "upload", label: "底表上传", icon: "⬆️" },
      { view: "versions", label: "版本管理", icon: "📝" },
      { view: "diffs", label: "版本对比", icon: "↔️" },
    ],
  },
  {
    label: "Support",
    items: [
      { view: "rules", label: "规则口径", icon: "⚙️" },
    ],
  },
];
      ...
      <aside class="sidebar">
        <div class="brand">
          ...
          <div class="brand-text">
            <div class="brand-title">BOM-v3</div>
            <div class="brand-sub">产品成本管控</div>
          </div>
        </div>
        <nav class="nav">
          ${NAV_GROUPS.map((group) => `
            <div class="nav-group">
              <div class="nav-label">${h(group.label)}</div>
              ${group.items.map((item) => navButton(item.view, item.label, item.icon)).join("")}
            </div>
          `).join("")}
        </nav>
      </aside>
...
function navButton(view, label, icon) {
  ...
  return `<button class="${active}" data-view="${view}"><span class="nav-icon">${icon}</span>${h(label)}</button>`;
}
  ...
  return `
    <div class="topbar">
      <div class="topbar-left">
        <h1>${h(title)}</h1>
        <div class="topbar-subtitle">${h(subtitle)}</div>
      </div>
...
function renderDashboard() {
  ...
  const k = data.kpis || {};
  const versionStatus = data.version ? statusText(data.version.status) : "-";
  const diffRatioVal = k.diff_ratio || 0;
  const diffPill = diffRatioVal > 0 ? '<span class="kpi-pill down">上涨</span>' : diffRatioVal < 0 ? '<span class="kpi-pill up">下降</span>' : '<span class="kpi-pill neutral">持平</span>';
  shell(`
    ${header("成本看板", "展示产品总成本、工段结构、版本趋势、缺价风险和差异明细")}
    <section class="grid cols-4">
      ${kpiCard("产品总成本", money(k.total_cost), "💰", '<span class="kpi-pill neutral">当前版本</span>')}
      ${kpiCard("较上一版本差异", money(k.diff_amount), "📈", diffPill)}
      ${kpiCard("差异比例", pct(k.diff_ratio), "📉", diffPill)}
      ${kpiCard("缺失价格物料数", k.missing_price_count ?? 0, "⚠️", (k.missing_price_count ?? 0) > 0 ? '<span class="kpi-pill down">待处理</span>' : '<span class="kpi-pill up">正常</span>')}
    </section>
    <section class="grid cols-4">
      ${kpiCard("最大成本工段", k.max_cost_stage || "-", "🏭", '<span class="kpi-pill neutral">占比最高</span>')}
      ${kpiCard("最大上涨工段", k.max_increase_stage || "-", "🔺", '<span class="kpi-pill neutral">波动最大</span>')}
      ${kpiCard("成本口径", state.costBasis === "tax_excluded" ? "不含税" : "含税", "🧾", '<span class="kpi-pill neutral">已切换</span>')}
      ${kpiCard("版本状态", versionStatus, "🏷️", data.version ? statusPill(data.version.status) : '<span class="kpi-pill neutral">-</span>')}
    </section>
    <section class="panel">
      <div class="toolbar">
        <div class="toolbar-title"><span class="icon">📋</span>工段成本汇总</div>
    ...
    <section class="panel">
      <div class="toolbar">
        <div class="toolbar-title"><span class="icon">🚨</span>缺价风险明细</div>
        <span class="muted">允许底表先入库，但阻断成本版本确认/发布</span>
      </div>
    ...
    <section class="grid cols-2">
      <div class="panel">
        <div class="toolbar">
          <div class="toolbar-title"><span class="icon">📊</span>成本趋势</div>
        </div>
        ${trendList(data.trend || [])}
      </div>
      <div class="panel">
        <div class="toolbar">
          <div class="toolbar-title"><span class="icon">🔍</span>版本差异 Top 20</div>
        </div>
...
function kpiCard(label, value, icon, pill) {
  return `
...
function stageTable(rows) {
  ...
  return `
    <div class="table-wrap"><table>
      ...
      <tbody>${rows.map((row) => `
        <tr>
          <td><strong>${h(row.stage)}</strong></td>
  ...
  return rows.map((r) => `
    <div style="margin-top:14px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
        <span style="font-size:13px;color:var(--text-secondary);">${h(r.version_name)}</span>
        <strong style="font-size:14px;">${money(r.cost)}</strong>
      </div>
...
function uploadResult() {
  ...
  return `
    <section class="panel">
      <div class="toolbar">
        <div class="toolbar-title"><span class="icon">📤</span>上传结果</div>
        ${result.version ? statusPill(result.version.status) : ""}
      </div>
      <div class="grid cols-4">
        ${kpiCard("SKU", summary.sku || result.version?.sku || "-", "📦", '<span class="kpi-pill neutral">SKU</span>')}
        ${kpiCard("行数", summary.row_count ?? "-", "📑", '<span class="kpi-pill neutral">总行数</span>')}
        ${kpiCard("末级数", summary.leaf_count ?? "-", "🌿", '<span class="kpi-pill neutral">末级物料</span>')}
        ${kpiCard("缺价数", summary.missing_price_count ?? result.version?.missing_price_count ?? "-", "⚠️", '<span class="kpi-pill neutral">待补价</span>')}
      </div>
...
function issuesTable(title, rows) {
  ...
  return `
    <div style="margin-top:20px;">
      <div style="font-size:15px;font-weight:600;color:var(--text-primary);margin-bottom:12px;">${h(title)}</div>
      <div class="table-wrap"><table>
...
function renderVersions() {
  shell(`
    ...
    <section class="panel">
      <div class="toolbar">
        <div class="toolbar-title"><span class="icon">📝</span>版本列表</div>
      </div>
      ${versionTable()}
    </section>
    <section class="panel">
      <div class="toolbar">
        <div class="toolbar-title"><span class="icon">🔎</span>成本明细</div>
  ...
  return `
    <div class="table-wrap"><table>
      ...
      <tbody>${state.versions.map((v) => `
        <tr>
          <td><strong>${h(v.sku)}</strong></td><td>${h(v.version_name)}</td><td>${statusPill(v.status)}</td>
          ...
          <td>
            <div style="display:flex;align-items:center;gap:6px;">
              <button class="secondary compact" data-recalc="${h(v.version_id)}">复算</button>
              <button class="compact" data-confirm="${h(v.version_id)}" ${v.missing_price_count ? "disabled" : ""}>确认</button>
              <button class="danger compact" data-delete-version="${h(v.version_id)}" data-version-name="${h(v.version_name)}">删除</button>
            </div>
          </td>
        </tr>
      `).join("")}</tbody>
  ...
  return `
    <div class="table-wrap"><table>
      ...
      <tbody>${pageItems.map((i) => `
        <tr>
          <td>${h(i.level)}</td><td>${i.is_leaf ? "是" : "否"}</td><td><code style="font-family:inherit;font-weight:600;">${h(i.material_code)}</code></td><td>${h(i.material_name)}</td><td>${h(i.stage)}</td>
...
function detailPager(total, totalPages, currentPage, start, end) {
  return `
    <div class="pagination">
      ...
      <div style="display:flex;align-items:center;gap:10px;">
        <button class="secondary compact" data-detail-page="prev" ${currentPage <= 1 ? "disabled" : ""}>上一页</button>
        <span class="page-indicator">第 ${h(currentPage)} / ${h(totalPages)} 页</span>
        <button class="secondary compact" data-detail-page="next" ${currentPage >= totalPages ? "disabled" : ""}>下一页</button>
      </div>
    </div>
  `;
}
...
function renderDiffs() {
  shell(`
    ...
    <section class="panel">
      <div class="toolbar">
        <div class="toolbar-title"><span class="icon">↔️</span>差异明细</div>
      </div>
...
function diffTable(rows, editable) {
  ...
  return `
    <div class="table-wrap"><table>
      ...
      <tbody>${rows.map((d) => `
        <tr>
          <td><code style="font-family:inherit;font-weight:600;">${h(d.material_code)}</code></td><td>${h(d.material_name)}</td><td>${h(d.diff_type)}</td><td>${money(d.previous_cost)}</td><td>${money(d.current_cost)}</td><td>${money(d.diff_amount)}</td><td>${pct(d.diff_ratio)}</td><td>${h(d.auto_reason)}</td>
          ${editable ? `<td><textarea data-reason="${h(d.diff_id)}">${h(d.manual_reason || "")}</textarea><div class="muted" style="margin-top:4px;font-size:12px;">${h(d.edited_by || "")} ${h((d.edited_at || "").slice(0, 19))}</div></td><td><button class="compact" data-save-reason="${h(d.diff_id)}">保存</button></td>` : ""}
        </tr>
      `).join("")}</tbody>
    </table></div>
  `;
...
function renderRules() {
  shell(`
    ...
    <section class="panel">
      <div class="toolbar">
        <div class="toolbar-title"><span class="icon">⚙️</span>业务规则</div>
      </div>
      <div class="table-wrap"><table>
